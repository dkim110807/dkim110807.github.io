...
constexpr int MAX = 200'002;

int color[MAX + 1], dist[MAX + 1], grundy[MAX + 1], parent[MAX + 1];
...
void dfs2(int v, int p = -1) {
    ...
    std::set<int> track;
    for (auto &x: child) {
        if (sets[x].count(dist[parent[v]])) {
            track.insert(g ^ grundy[x]);
        }
    }
    if (color[v] == 0) track.insert(g);

    int mex = 0;
    while (track.count(mex)) mex += 1;
    grundy[v] = mex;

    for (const auto &x: child) {
        if (sets[v].size() < sets[x].size()) std::swap(sets[v], sets[x]);
        sets[v].insert(sets[x].begin(), sets[x].end());
    }
    sets[v].insert(dist[v]);
}
...
int main() {
    ...
    std::cout << (grundy[1] ? "kidw0124" : "eoaud0108");
}
```
...
int main() {
    ...
    for (int i = 0; i < 10; i++) {
        if (p[i] == 0 && q == 0) {
            dp[i][1] = p[i] + (1 - p[i]) * (3 - q) / (1 - q);
        } else {
            dp[i][1] = (p[i] * (2 - q - p[i] * (1 - q)) + q * (2 - 2 * p[i])) /
                       ((p[i] + q - p[i] * q) * (p[i] + q - p[i] * q));
            if (q != 1) dp[i][1] = std::min(dp[i][1], p[i] + (1 - p[i]) * (3 - q) / (1 - q));
        }
    }

    for (int i = 2; i <= n; i++) {
        for (int k = 0; k < 10; k++) dp[k][i] = p[k] * dp[k][i - 1] + (1 - p[k]) * dp[k][i - 2] + 1;
    }
...
와 같이 표현할 수 있다면, $T$라는 변환을 통해서 원소별 곱셈으로 바꿀 수 있으며, 역변환으로 복원 가능하다. xor 연산의 경우
...
> 각 재료별로 행렬을 모두 찾아 이를 Kronecker Product 하는 방법으로, 행렬을 항등 행렬과의 텐서 곱의 행렬 곱으로 생각하는 풀이도 가능하다. 사실상 FWHT와 동일하다.
{: .prompt-tip }
